...
9. Opening via the dedicated calendar icon timed out the same way the input did - so the click target was never really the problem. This widget's JS attachment is just inherently flaky under CI load. Retrying the *original* input click patiently (up to 30 seconds) - roughly what the one successful early run got "for free" from favorable timing - is what finally, reliably got a real booking through: the confirmation page rendered with the correct facility, program, date, and comment, every field intact.
10. One last, much smaller bug surfaced once the booking itself became reliable: `getConfirmedFacility()` read the confirmed facility name via an xpath assuming a `<table>` layout that was never real. The actual confirmation page pairs `<label for="facility">` with `<p id="facility">` - confirmed from this run's own captured HTML. Fixed to read `#facility` directly.
