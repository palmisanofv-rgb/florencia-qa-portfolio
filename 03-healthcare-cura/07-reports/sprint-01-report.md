...
Two further CI runs each failed on the same test, one layer deeper each time. First, the visit-date field was being sent `10/15/2026` (US mm/dd/yyyy ordering) against a field whose own placeholder reads `dd/mm/yyyy` — switching to `15/10/2026` was the natural first fix. That still failed: a follow-up screenshot showed the field back to empty (still showing its `dd/mm/yyyy` placeholder) with the calendar popup open. The field is backed by a Bootstrap datepicker that doesn't accept real typed input at all — `sendKeys()` only ever triggered the popup, never landed a value, and the "fix" of pressing Escape just closed that popup with the field still empty, so the form submission was booking with no date. The real fix was setting the field's value directly via JavaScript, which never opens the popup in the first place.
