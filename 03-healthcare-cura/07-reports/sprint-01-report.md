...
8. The actual fix targets that real parent container — `$('#txt_visit_date').closest('.input-group.date').datepicker('setDate', ...)` — and fires `changeDate` on it. As of this report, CI verification of this attempt is still in progress.
