...
Automation ratio is a *deliberate* number in this portfolio, not "everything I got around to." Project 01 automates 5 of 9 P1 scenarios end-to-end, and every single unautomated one is a checkout/payment variant, kept manual on purpose — the sandbox's payment form isn't idempotent across runs, and in a real project with a live payment gateway, that decision would need to be revisited before a production release rather than left manual indefinitely. That the gap is 100% concentrated in one deliberately-scoped-out family of scenarios, rather than scattered, is what makes this a judgment call rather than a shortfall — an automation ratio that reflects judgment, not just effort.
